...
กำหนด frontmatter spec สำหรับ workflow `.md` files — ครอบคลุม 26 fields แบ่งเป็น 6 กลุ่ม: Core, Classification, Safety, Execution Permissions, Execution Control, และ Output & Lifecycle
...
4. ตั้ง `reversible` เป็น `true` ถ้าการเปลี่ยนแปลงย้อนกลับได้ เช่น `/git-revert`, `false` ถ้าย้อนกลับไม่ได้ เช่น `/delete`
5. ถ้า `deprecated: true` ต้องมี `replaces` หรือระบุ workflow ที่ควรใช้แทนใน body
...
3. ตั้ง `max_retries` เฉพาะเมื่อ retry limit ต่างจาก default `3` — เช่น `/watch-github-actions` อาจตั้ง `5`
4. ตั้ง `timeout` เป็นจำนวนวินาทีสูงสุดที่ workflow ควรรัน — default ไม่มี ถ้าไม่ใส่หมายถึงไม่จำกัด
5. ตั้ง `ai_tools` เฉพาะ tools ที่ format นี้รองรับจริง — ถ้าไม่ใส่หมายถึงรองรับทุก tools
...
| `reversible` | boolean | no | `true`, `false` |
...
| `ai_tools` | array | no | `windsurf`, `codex`, `claude`, `opencode` |
| `max_retries` | number | no | integer >= 0, default `3` |
...
| `url` | string | no | external docs URL |
...
max_retries: 3
timeout: 60
updated_last: 2026-07-14T06:30:00+07:00
allow:
  - file-read
interactive: false
auto_run: always
...
- Workflow frontmatter มี 26 fields ครบ — 8 required + 18 optional
