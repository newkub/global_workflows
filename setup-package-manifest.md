...
ตั้งค่า scripts ใน `package.json` หรือ `Cargo.toml` ตามมาตรฐานที่กำหนด รองรับทั้ง Minimal, Standard และ Complete levels เพื่อใช้งานร่วมกับ `run-verify` สำหรับการตรวจสอบคุณภาพโค้ดครบวงจร
...
- Lefthook hooks จะรันอัตโนมัติก่อน commit, push, rebase (ต้องรัน `setup-lefthook` ก่อน)
