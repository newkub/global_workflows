---
title: Analyze Bug Prone
description: วิเคราะห์ code ที่มีแนวโน้มก่อให้เกิด bug ก่อนเกิดปัญหา
...
วิเคราะห์ code เพื่อหาจุดที่มีแนวโน้มก่อให้เกิด bug ก่อนที่ปัญหาจะเกิดขึ้น

## Scope

ใช้สำหรับวิเคราะห์ code ที่ดูเหมือนทำงานได้แต่อาจมี subtle bugs ซ่อนอยู่ ครอบคลุม: logic errors, null/undefined access, async pitfalls, state mutation, numeric/date pitfalls, edge cases, resource leaks, และ type-runtime mismatches
...
- ทำ analysis เท่านั้น ไม่แก้ไข code ระหว่าง analysis
- ใช้ `/comment-todo` สำหรับระบุ issues ใน code
- ถ้าต้องแก้ไข ให้ทำ `/resolve-issue` หลัง analysis
...
- ไม่มี code changes ระหว่าง analysis
- สามารถนำ findings ไปทำ `/resolve-issue` ได้โดยตรง
