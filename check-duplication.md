...
1. รัน `bunx jscpd . --min-lines 5 --min-tokens 50 --threshold 10 --reporters console,json --output report`
2. ทำ `/use-scripts` เพื่อ parse jscpd JSON output, generate custom reports, และ group results ใน script เดียว
3. ระบุไฟล์และส่วนที่มี duplication
