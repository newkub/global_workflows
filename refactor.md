...
วิเคราะห์โปรเจกต์และวางแผน refactor

1. อ่าน `/restructure` ก่อนเริ่ม refactor
2. ทำ `/analyze-structure` เพื่อดูโครงสร้างไฟล์และ folders
2. ทำ `/analyze-code-quality` เพื่อระบุ SRP, SoC, type safety, anti-patterns, code smells, dead code, side effects, naming conventions
3. ตรวจสอบ `package.json` สำหรับ `workspaces` field
4. ตรวจสอบ `turbo.json` หรือ `moon.yml` สำหรับ monorepo setup
5. ระบุว่าเป็น monorepo หรือ single workspace
